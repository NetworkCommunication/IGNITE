"""
@File: main_different_cn.py
@author: Zijia Zhao
@Describtion: Comparative experiments of different task sizes
"""
...
def gk_ddpg(episode, filename, fn_car, fn_bs, cn, step):
    ...
    df_cost.to_excel("gk_ddpg/task_cn/{}.xlsx".format(filename + '_gk_ddpg'))
...
def gk_dqn(episode, filename, fn_car, fn_bs, cn, step):
    ...
    df_cost= pd.DataFrame({"Episode": episode_record_step, "Cost": cost_record_step}).set_index('Episode')
    df_cost.to_excel("gk_dqn/task_cn/{}.xlsx".format(filename + '_gk_dqn'))
...
def gk_ddpg_local(episode, filename, fn_car, fn_bs, cn, step):
    ...
    df_cost = pd.DataFrame({"Episode": episode_record_step, "Cost": cost_record_step}).set_index('Episode')
    df_cost.to_excel("gk_ddpg_local/task_cn/{}.xlsx".format(filename + '_gk_ddpg_local'))
...
def gk_ddpg_full(episode, filename, fn_car, fn_bs, cn, step):
    ...
    df_cost = pd.DataFrame({"Episode": episode_record_step, "Cost": cost_record_step}).set_index('Episode')
    df_cost.to_excel("gk_ddpg_full/task_cn/{}.xlsx".format(filename + '_gk_ddpg_full'))
...
if __name__ == '__main__':
    ...
    fn_car = 0.5e8
    cn = cn_input * 1e8
    filename = str(round) + '_' + str(cn_input) + '_task_cn'
